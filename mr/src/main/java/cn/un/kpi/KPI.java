...
public class KPI {
    ...
    private static KPI parser(String line) {
        ...
        if (fields.length > 11) {
            kpi.setRemote_addr(fields[0]);
            kpi.setRemote_user(fields[1]);
            kpi.setTime_local(fields[3].substring(1));
            kpi.setRequest(fields[6]);
            kpi.setStatus(fields[8]);
            kpi.setBody_bytes_sent(fields[9]);
            kpi.setHttp_referer(fields[10]);

            if (fields.length > 12) {
                kpi.setHttp_user_agent(fields[11] + " " + fields[12]);
...
//                kpi.setHttp_user_agent(sb.toString());
                ...
                kpi.setHttp_user_agent(fields[11]);
            }
